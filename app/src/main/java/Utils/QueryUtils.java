...
import com.example.android.quakereport.EarthquakeListing;
...
public final class QueryUtils {
    ...
    /**
     * Return a list of {@link EarthquakeListing} objects that has been built up from
     * parsing a JSON response.
     */
    public static ArrayList<EarthquakeListing> extractEarthquakes() {

        // Create an empty ArrayList that we can start adding earthquakes to
        ArrayList<EarthquakeListing> earthquakes = new ArrayList<>();
        ...
        try {
            ...
            JSONObject resJSONObject = new JSONObject(SAMPLE_JSON_RESPONSE);

            JSONArray featureJsonArray = resJSONObject.getJSONArray("features");
            for (int i = 0; i <= featureJsonArray.length(); i++) {
                JSONObject earthJsonOBJ = featureJsonArray.getJSONObject(i);
                JSONObject propertJSONObj = earthJsonOBJ.getJSONObject("properties");

                earthquakes.add(new EarthquakeListing(propertJSONObj.getString("mag"),
                        propertJSONObj.getString("place"),propertJSONObj.getString("time")));
            }
